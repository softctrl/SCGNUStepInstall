...
# Install ICU support
wget -c "http://download.icu-project.org/files/icu4c/52rc/icu4c-52_rc2-src.tgz" && tar -zxvf icu4c-52_rc2-src.tgz && cd icu/source && ./configure  && gmake && gmake install && cd ../.. && rm -rvf icu/
cp -Rvf /usr/local/lib/libicu* /usr/lib64/
...
wget -c "ftp://ftp.gnustep.org/pub/gnustep/core/gnustep-base-1.24.6.tar.gz" && tar -zxvf gnustep-base-1.24.6.tar.gz && cd gnustep-base-1.24.6 && ./configure && make && make install && cd .. && rm -rvf gnustep-base-1.24.6/
cp -Rvf /usr/local/lib/libgnustep-base* /usr/lib64/
